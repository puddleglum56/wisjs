...
function SearchBar() {
  ...
  <FormControl className="search-select" sx={{minWidth: "30%"}}>
    ...
    <TextField onChange={(event) => textFieldValue.current = event.target.value} variant="outlined" size="small" className="search-input" sx={{backgroundColor: "white", minWidth: "40%"}}/>
    <Button onClick={() => dispatch(setSearch(textFieldValue.current))} variant="contained" className="search-button" sx={{marginLeft: "1vw", maxWidth: "5%"}}>Search</Button>
  </>
  );
}
